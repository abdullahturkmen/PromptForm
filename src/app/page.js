...
export default function Home() {
  ...
  const getResponse =  async () => {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [{
        role: 'system',
        content: 'Sen html olarak çıktı veren bir web form üretme aracısın, kullanıcılara direkt kod ver, kodlar css yönünden şık olsun, bootstrap veya tailwind de kullanabilirsin bunları oluştururken'
      }, {
        role: "user",
        content: text
      }],
    ...
    setResponseData(response.choices[0].message.content)
         setButtonLoading(false)
  ...
  const copyHTML = () => {
    ...
    var copyCode = responseData
    if (responseData.includes('```')) {
      copyCode = responseData.split('```')[1]
      copyCode = copyCode.slice(4)
    }
  ...
  return (
    <div className="flex flex-col container mx-auto">
      <div className="bg-white  mx-auto my-3 shadow-md rounded px-3 lg:px-8 pt-6 pb-8 mb-4 w-11/12 lg:w-6/12 ">
        ...
        {buttonLoading ? (<><button disabled class="text-white bg-gray-400  focus:ring-4  font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2  inline-flex items-center">
        ...
        {responseData && (
          <div>
            {/* <iframe
            className="html-content my-10"
            srcDoc={responseData}
            style={{ width: '100%', height: '400px', border: 'none' }}
          /> */}
            <iframe className="my-10" srcDoc={responseData} style={{ width: '100%', height: '500px', border: 'none' }} />
            <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline" onClick={copyHTML}>Kodları Kopyala</button>
          </div>
        )}
      </div>
      <ToastContainer />
